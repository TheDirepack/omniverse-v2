...
class PredicateService:
    ...
    def normalize(self, raw_predicate: str) -> str:
        ...
        try:
            ...
            alias = session.exec(
                select(PredicateAlias).where(PredicateAlias.alias == normalized_raw)
            ).first()
            ...
            pred = session.exec(
                select(Predicate).where(Predicate.canonical_name == normalized_raw)
            ).first()
            if pred:
    ...
    def upsert_alias(self, alias: str, canonical_name: str):
        ...
        try:
            ...
            pred = session.exec(
                select(Predicate).where(Predicate.canonical_name == canonical_name)
            ).first()
            if not pred:
            ...
            existing_alias = session.exec(
                select(PredicateAlias).where(PredicateAlias.alias == normalized_alias)
            ).first()
            if existing_alias:
                ...
                session.add(
                    PredicateAlias(alias=normalized_alias, predicate_id=pred.id)
                )
