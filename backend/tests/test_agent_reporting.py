...
async def test_execute_plan_logs_steps():
        ...
        "function": {
            ...
            "arguments": json.dumps({
                "plan": [
                    ...
                    {"tool": "upsertClaims", "args": {"subject": "Test", "predicate": "is", "object_val": "True"}}
                ]
            }),
        },
    }
    ...
    with patch("app.core.router.router.run_model", new_callable=AsyncMock) as mock_run_model, \
         patch("app.core.agent_engine.agent_logger.log") as mock_log, \
         patch("app.core.agent_engine._execute_tool", new_callable=AsyncMock) as mock_exec_tool:
