...
2. Query the unconfirmed staging traits (`queryUnconfirmedTraits`) — these carry richer metadata (category, canon_status, reference, wiki_source, confidence) than the freeform research text and are the authoritative record of what the Researcher actually found and cited.
3. Plan the merge (Update X, Create Y, Delete Z), preferring the staged, cited version of a fact over the freeform summary when they differ.
4. Execute the changes using `upsertTrait`.
