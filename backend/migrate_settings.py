...
def migrate():
    ...
    with Session(main_engine) as main_session, Session(settings_engine) as set_session:
        ...
        for p in providers:
            set_session.add(ProviderConfig(
        ...
        for k in keys:
            set_session.add(ProviderKey(
                id=k.id, provider_id=k.provider_id, 
                api_key=k.api_key, priority=k.priority
            ))
            
        print("Migrating Agent Routes...")
        routes = main_session.exec(select(AgentRouteFallback)).all()
        for r in routes:
            set_session.add(AgentRouteFallback(
                id=r.id, task_type=r.task_type, priority=r.priority, 
                provider_id=r.provider_id, models=r.models
            ))
            
        print("Migrating Model Configs...")
        m_configs = main_session.exec(select(ModelConfig)).all()
        for mc in m_configs:
            set_session.add(ModelConfig(id=mc.id, model_name=mc.model_name, provider_id=mc.provider_id))

        print("Migrating Candidate Health...")
        health = main_session.exec(select(CandidateHealth)).all()
        for h in health:
            set_session.add(CandidateHealth(
                candidate_hash=h.candidate_hash, provider_id=h.provider_id, 
                key_id=h.key_id, model=h.model, failure_count=h.failure_count, 
