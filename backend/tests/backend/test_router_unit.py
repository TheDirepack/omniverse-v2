...
from app.core.router import ModelRouter, _clean_error, calculate_candidate_hash
...
class TestModelRouterHealth:
    ...
    def test_report_failure_threshold_disables(self):
        ...
        assert health.disabled_until > datetime.now(timezone.utc)
    ...
    def test_report_success(self):
        ...
        health = CandidateHealth(
            candidate_hash="hash", provider_id=1, key_id=10, model="gpt-4",
            failure_count=5, disabled_until=datetime.now(timezone.utc) + timedelta(hours=1)
        )
...
class TestModelRouterRouting:
    ...
    async def test_independence_guard(self, router_inst, mock_session):
        with patch("app.core.router.litellm.acompletion", new_callable=AsyncMock) as mock_completion, \
            ...
            ProviderKey(id=10, api_key="k1", priority=0)
            ...
            ProviderConfig(id=1, provider_type="openai", base_url="url1", models="gpt-4")
