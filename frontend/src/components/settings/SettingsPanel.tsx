	...
	return (
		<section className="panel-grid settings-grid">
			<div className="settings-tabs">
				<button
					className={settingsTab === "providers" ? "tab active" : "tab"}
					onClick={() => setSettingsTab("providers")}
				>
					Providers & Keys
				</button>
				<button
					className={settingsTab === "routing" ? "tab active" : "tab"}
					onClick={() => setSettingsTab("routing")}
				>
					Agent Routing
				</button>
				<button
					className={settingsTab === "general" ? "tab active" : "tab"}
			...
			{settingsTab === "providers" && (
				<div className="settings-content">
					...
					<p className="muted">
						Define LLM providers and their fallback API keys.
					</p>
					<button className="primary" onClick={handleAddProvider}>
						+ Add Provider
					</button>
			...
			{settingsTab === "general" && (
				<div className="settings-content">
					...
					<p className="muted">
						Free-form key/value pairs used by custom agent prompts or
						integrations.
					</p>
					...
					>
						...
						>
							Models that fail repeatedly are temporarily disabled (circuit
							breaker).
						</p>
						<button
							className="primary"
							onClick={async () => {
								if (confirm("Reset all model timeouts and failure counts?")) {
									try {
										await api.resetCandidateHealth();
										alert("Model health reset successfully.");
									} catch (e) {
										console.error(e);
									}
								}
					...
					<div className="add-setting-row">
						<input
							...
							onKeyDown={(e) => {
								if (e.key === "Enter") void handleAddSetting();
							}}
						...
						<button
							...
							disabled={savingAddKey || !newSettingKey.trim()}
						>
					...
					<div className="settings-list">
						{Object.entries(settings).map(([key, value]) => {
							if (
								key === "AGENT_LOGGING" ||
								key === "HIDE_WEBFETCH_CONTENT" ||
								key === "HIDE_WEBSEARCH_CONTENT"
							) {
								return (
									<SettingToggle
										key={key}
										keyName={key}
										value={
											value ?? (key === "AGENT_LOGGING" ? "true" : "false")
										}
										onSave={handleSaveSetting}
									/>
								);
