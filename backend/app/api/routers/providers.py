...
@router.post("/")
def upsert_provider(payload: ProviderPayload, service: SettingsService = Depends(get_settings_service)):
    return service.upsert_provider(payload.model_dump())

@router.get("/{provider_id}/models")
async def get_provider_models(provider_id: int, service: SettingsService = Depends(get_settings_service)):
...
@router.post("/keys")
def upsert_provider_key(payload: ProviderKeyPayload, service: SettingsService = Depends(get_settings_service)):
...
def delete_provider_key(key_id: int, service: SettingsService = Depends(get_settings_service)):
    if not service.delete_provider_key(key_id):
        raise HTTPException(status_code=404, detail="Provider key not found")
    return {"status": "success"}

@router.delete("/{provider_id}")
def delete_provider(provider_id: int, service: SettingsService = Depends(get_settings_service)):
