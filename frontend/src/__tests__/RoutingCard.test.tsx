...
const mockProviders = [
	{
		...
		keys: [{ id: 1, provider_id: 1, api_key: "sk-...", priority: 0 }],
	},
	{
		id: 2,
		name: "Anthropic",
		...
		keys: [{ id: 2, provider_id: 2, api_key: "sk-ant-...", priority: 0 }],
	},
];
...
describe("RoutingCard", () => {
	...
	it("renders agent name and status text for non-DEFAULT with no custom routes", () => {
		render(
			<RoutingCard
				...
				onSave={mockOnSave}
				onDelete={mockOnDelete}
				defaultRoutes={mockDefaultRoutes}
			/>,
		);
		expect(screen.getByText("Researcher")).toBeInTheDocument();
		expect(
			screen.getByText(/Using DEFAULT \(1 step\(s\)\)/),
		).toBeInTheDocument();
	});
	...
	it("expands and collapses on header click", async () => {
		...
		await user.click(screen.getByText("DEFAULT").closest("div") as HTMLElement);
		expect(
			screen.queryByText("Resolved Fallback Order:"),
		).not.toBeInTheDocument();
	});

	it("shows resolved fallback order when expanded", async () => {
		const user = userEvent.setup();
		render(
			<RoutingCard
				agentName="DEFAULT"
		...
		expect(screen.getByText("Resolved Fallback Order:")).toBeInTheDocument();
		await user.click(screen.getByText("Resolved Fallback Order:"));
	...
	it("calls onDelete when delete button is clicked", async () => {
		...
		const slot = screen.getByText("#1").closest(".route-slot") as HTMLElement;
		const deleteBtn = within(slot).getByRole("button", { name: "×" });
	...
	it("calls onSave to add a new slot when + Add Provider Step is clicked", async () => {
		...
		await user.click(
			screen.getByRole("button", { name: /\+ add provider step/i }),
		);
		expect(mockOnSave).toHaveBeenCalledWith({
			id: 0,
			task_type: "DEFAULT",
		...
		); // 2 for slot 1 (provider+model), 1 for new slot (provider only)
	});
});
