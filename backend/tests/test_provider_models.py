...
async def test_fetch_live_models(ptype: str):
    ...
    if _requires_key(ptype) and not creds["api_key"]:
        pytest.skip(
            f"provider '{ptype}' not configured (set api_key in provider_config.py)"
        )
    ...
    with Session(engine) as session:
        ...
        if creds["api_key"]:
            key = ProviderKey(
                provider_id=provider.id, api_key=creds["api_key"], priority=0
            )
            session.add(key)
    ...
    for m in models:
        assert isinstance(m, str) and m.strip(), (
            f"empty or non-string model name: {m!r}"
        )
