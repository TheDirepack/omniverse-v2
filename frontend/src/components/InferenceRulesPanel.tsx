...
import type { InferenceRule, Contradiction } from "../types";
...
const EMPTY_RULES: RulesByStatus = {
	proposed: [],
	critiqued: [],
	approved: [],
	rejected: [],
			...
			>
				<div>
					<div className="audit-label">
						Proposer ({rule.proposer_model ?? "unknown"})
					</div>
				...
				<div>
					...
					<div className="audit-text">
						{rule.critic_rationale ?? "— not yet critiqued —"}
					</div>
				</div>
			</div>
				...
				<div style={{ display: "flex", gap: 8 }}>
					...
					{onReject && (
						<button
							...
							style={{ padding: "4px 12px", cursor: "pointer", opacity: 0.7 }}
						>
							Reject
						</button>
					)}
...
function InferenceRulesPanel() {
	...
	const handlePropose = async () => {
		...
		try {
			...
			alert(
				"Rule proposal pass started. Refresh in a moment to see results — this calls two separate models (proposer + independent critic) per candidate pair.",
			);
		} catch (e) {
	...
	const handleMaterialize = async () => {
		...
		try {
			...
			alert(
				`Materialization complete. ${result.created_count} new inferred claim(s) created.`,
			);
			await refresh();
		} catch (e) {
		...
		<section className="panel-grid single">
			<div className="panel">
				...
				<p className="help-text">
					Path-level composition rules, proposed by one model and independently
					critiqued by another. Nothing here is ever auto-approved — a bad
					composition rule silently corrupts every inference derived from it, so
					a human must approve before it's used. Nothing here runs automatically
					either; every pass below is manually triggered.
				</p>
				...
				>
					<div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
						<label htmlFor="max-depth" style={{ fontSize: "0.8rem", opacity: 0.8 }}>
					...
					>
						{materializing
							? "Materializing..."
							: `Materialize Paths (depth ${depth})`}
					</button>
					<button
						type="button"
						onClick={refresh}
						style={{ padding: "4px 12px", opacity: 0.7 }}
					>
						Refresh
					</button>
				</div>
				...
				{contradictions.length > 0 && (
					...
					>
						<h3 style={{ color: "#b45309", marginBottom: 8 }}>
							{contradictions.length} Unreviewed Contradiction
							{contradictions.length === 1 ? "" : "s"}
						</h3>
						<p className="help-text" style={{ marginBottom: 8 }}>
							An inferred claim conflicts with a directly asserted one. This may
							mean the asserted claim is a legitimate exception, not an error —
							these are flagged for review, never auto-resolved.
						</p>
						{contradictions.map((c: Contradiction) => (
							...
							>
								Inferred claim #{c.id}: {c.predicate} — conflicts with claim #
								{c.contradicts_claim_id}
							</div>
						))}
					</div>
				)}
				...
				{rules.proposed.length > 0 && (
					<>
						<h2 style={{ fontSize: "1rem", marginBottom: 8, marginTop: 24 }}>
							Proposed (critique in progress)
						</h2>
