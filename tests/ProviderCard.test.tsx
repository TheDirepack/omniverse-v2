...
describe("ProviderCard", () => {
  ...
  it("renders provider name and type badge", () => {
    ...
    expect(screen.getByText((content, element) => element?.tagName.toLowerCase() === 'span' && content === 'OpenAI')).toBeInTheDocument();
  });
  ...
  it("shows base URL section for custom type and saves it", async () => {
    ...
    expect(screen.getByText((content, element) => element?.tagName.toLowerCase() === 'h4' && content === 'Base URL')).toBeInTheDocument();
    ...
    const baseUrlHeader = screen.getByText((content, element) => element?.tagName.toLowerCase() === 'h4' && content === 'Base URL');
    const baseUrlSection = baseUrlHeader.closest('.provider-section')!;
    const baseSaveBtn = within(baseUrlSection).getByRole("button", { name: /^save$/i });
  ...
  it("removes a model tag when × is clicked on the tag", async () => {
    ...
    const gpt4tag = screen.getByText("gpt-4").closest("span")!;
  ...
  it("shows warning badge when provider has no keys", () => {
    render(<ProviderCard provider={makeProvider({ keys: [] })} onSave={onSave} onSaveKey={onSaveKey} onDeleteKey={onDeleteKey} onDeleteProvider={onDeleteProvider} />);
  ...
  it("calls fetchProviderModels on Sync Saved Models click and merges result", async () => {
    vi.mocked(api.fetchProviderModels).mockResolvedValue({ models: ["gpt-4-turbo", "o1"] });
    const user = userEvent.setup();
    render(<ProviderCard provider={makeProvider()} onSave={onSave} onSaveKey={onSaveKey} onDeleteKey={onDeleteKey} onDeleteProvider={onDeleteProvider} />);
    await user.click(screen.getByRole("button", { name: /sync saved models/i }));
    await waitFor(() => expect(api.fetchProviderModels).toHaveBeenCalledWith(1));
