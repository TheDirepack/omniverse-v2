...
describe("TheoriesPanel", () => {
			...
			worlds: [
				{
					...
					name: "Warhammer 40k",
					tier: 1,
					theory: "Chaos Gods are real",
					theory_audit: "Accepted",
					summary: null,
				...
				},
				{
					...
					name: "Star Wars",
					tier: 3,
					theory: "The Force is a cosmic energy field",
					theory_audit: null,
					summary: null,
					tier_justification: null,
					is_explored: true,
	...
	it("shows empty state when no theories generated", async () => {
		...
		await screen.findByText(
			"No theories generated yet. Use the trigger above to extrapolate interactions.",
		);
	});
	...
	it("triggers extrapolate API call with correct payload for 'all' scope", async () => {
		...
		vi.mocked(api.extrapolate).mockResolvedValue({
			run_id: "test-run",
			worlds: [],
		});
