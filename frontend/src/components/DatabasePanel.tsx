...
import type { Anomaly, Claim, Trait, UnconfirmedClaim, World } from "../types";
				...
				{claims.map((c, i) => (
					<div
						...
						style={{
							...
							padding: "8px",
							background: isUnconfirmed ? "#2d2a2e" : "#1e293b",
							borderRadius: 4,
							border: `1px solid ${isUnconfirmed ? "#4a3f4e" : "#334155"}`,
						}}
					>
						<span style={{ color: "#cbd5e1", fontSize: "0.9rem" }}>
							...
							{typeof c === "object" && "object_val" in c
								? c.object_val
								: (c as any).object_literal || "Unknown"}
						</span>
						<span
							style={{ fontSize: "0.7rem", color: "#94a3b8", marginLeft: 8 }}
						>
			...
			<div style={{ display: "grid", gap: 8 }}>
				{traits.map((t, i) => (
					...
					>
						...
						<span style={{ color: isUnconfirmed ? "#c084fc" : "#cbd5e1" }}>
							{t.value}
						</span>
					</div>
				))}
...
function ComparisonMatrix({
	...
	const relevantTraits = allTraits.filter((t) =>
		worldIds.includes(t.universe_id),
	);

	// Get unique trait names across all selected worlds
	const uniqueTraitNames = Array.from(
		new Set(relevantTraits.map((t) => t.name)),
	).sort();
					...
					{uniqueTraitNames.map((traitName) => {
						return (
							<tr key={traitName} style={{ borderBottom: "1px solid #334155" }}>
								<td
									style={{ padding: "12px", fontWeight: 600, color: "#cbd5e1" }}
								>
									{traitName}
								</td>
								{selectedWorlds.map((w) => {
									const trait = relevantTraits.find(
										(t) => t.universe_id === w.id && t.name === traitName,
									);
									return (
										<td
											key={w.id}
...
function WorldDetail({
	...
	anomalies: Anomaly[];
	traits: Trait[];
	unconfirmedTraits: any[];
	claims: Claim[];
	unconfirmedClaims: UnconfirmedClaim[];
}) {
	const worldAnomalies = anomalies.filter((a) => a.world_id === world.id);
	const worldTraits = traits.filter((t) => t.universe_id === world.id);
	const worldUnconfirmed = unconfirmedTraits.filter(
		(t) => t.universe_name === world.name,
	);
	const worldClaims = claims.filter((c) => c.universe_scope === world.id);
	const worldUnconfirmedClaims = unconfirmedClaims.filter(
		(c) => c.universe_name === world.name,
	);

	const groupedTraits = worldTraits.reduce(
		(acc, t) => {
	...
	const groupedUnconfirmed = worldUnconfirmed.reduce(
		(acc, t) => {
			const cat = t.category || "Other";
			if (!acc[cat]) acc[cat] = [];
			acc[cat].push(t);
	...
	return (
		<div>
			...
			>
				...
				>
					Tier{" "}
					{world.tier !== null && world.tier !== -1 ? world.tier : "Untiered"}
				</div>
			</div>
			<div className="detail-box">
				<div className="detail-label">Tier Justification</div>
				<pre className="detail-content">
					{world.tier_justification ?? "No tiering data yet."}
				</pre>
			</div>
			...
			<div style={{ marginTop: 24 }}>
				...
				<div style={{ marginTop: 16 }}>
					...
					{worldTraits.length === 0 && (
						<p className="muted">No structured traits found in main DB.</p>
					)}
				</div>
			</div>
			...
			<div style={{ marginTop: 24 }}>
				...
				<ClaimSection
					title="Staging Claims"
					claims={worldUnconfirmedClaims}
					isUnconfirmed
				/>
				<div style={{ marginTop: 16 }}>
					...
					{worldUnconfirmed.length === 0 && (
						<p className="muted">No unconfirmed traits found in staging DB.</p>
					)}
				</div>
			</div>
			...
			{worldAnomalies.length > 0 && (
				...
				>
					...
					{worldAnomalies.map((a, i) => (
						<pre key={i} className="detail-content">
							{a.description}
						</pre>
					))}
				</div>
...
function DatabasePanel() {
	...
	const [unconfirmedClaims, setUnconfirmedClaims] = useState<
		UnconfirmedClaim[]
	>([]);
	...
	const refresh = async () => {
		try {
			...
			const [allTraits, allClaims, allUnconfirmedTraits, allUnconfirmedClaims] =
				await Promise.all([
					api.fetchTraits(),
					api.fetchClaims(),
	...
	const compareWorlds = useMemo(
		() => worlds.filter((w) => compareIds.includes(w.id)),
		[worlds, compareIds],
	);

	const processedGrouped = useMemo(() => {
		const filtered = worlds.filter((w) =>
			w.name.toLowerCase().includes(worldFilter.toLowerCase()),
		);
		...
		<section className="panel-grid tiers">
			<div className="panel list">
				...
				<div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
					<button className="primary" onClick={handleRunTiering}>
						Run Tiering Pass
					</button>
					<button
						className={isCompareMode ? "primary" : ""}
						onClick={() => {
				...
				<div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
					...
					>
						<input
							type="checkbox"
							checked={showGlobalUnverified}
							onChange={(e) => setShowGlobalUnverified(e.target.checked)}
						/>
						Show Unverified
					</label>
					<label
					...
					>
						<input
							type="checkbox"
							checked={showGlobalTheories}
							onChange={(e) => setShowGlobalTheories(e.target.checked)}
						/>
						Show Theories
					</label>
				</div>
				...
				{!tierSystem && (
					<p className="muted">
						No tier system yet. Run the pipeline to generate one.
					</p>
				)}
				...
				{isCompareMode && (
					<div style={{ marginTop: 16 }}>
						<button className="secondary" onClick={() => setCompareIds([])}>
							Clear Selection
						</button>
			...
			<div className="panel detail">
				{isCompareMode ? (
					...
					<WorldDetail
						...
						anomalies={anomalies}
						traits={traits}
						unconfirmedTraits={unconfirmedTraits}
						claims={claims}
						unconfirmedClaims={unconfirmedClaims}
					/>
				) : (
					<p className="muted">Select a world to view details.</p>
				)}
				{anomalies.filter((a) => a.world_id === null).length > 0 && (
					<div className="anomaly-list" style={{ marginTop: 16 }}>
						<h4>Global Anomalies</h4>
						{anomalies
							.filter((a) => a.world_id === null)
							.map((a, i) => (
						...
						<div style={{ display: "grid", gap: 16 }}>
							<div>
								...
								{unconfirmedClaims.length > 0 ? (
									<ClaimSection
										title="All Staging Claims"
										claims={unconfirmedClaims}
										isUnconfirmed
									/>
								) : (
							...
							<div>
								...
								{unconfirmedTraits.length > 0 ? (
									<TraitSection
										category="All Unconfirmed"
										traits={unconfirmedTraits}
										isUnconfirmed
									/>
								) : (
				...
				{showGlobalTheories && (
					...
					>
						...
						<div style={{ display: "grid", gap: 16 }}>
							...
							{worlds.filter((w) => w.theory).length === 0 && (
								<p className="muted">No theories available.</p>
							)}
						</div>
					</div>
				)}
