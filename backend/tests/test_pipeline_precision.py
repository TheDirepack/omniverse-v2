        ...
        with patch.object(retriever, 'get_universe_knowledge_graph') as mock_graph:
            mock_graph.return_value = {
                "Hero": {
                    "entity": "Hero",
                    "facts": [
                        {"predicate": "has_power", "object": "Flight",
                         "support": 5, "status": "VERIFIED", "reference": "ref1"}
                    ],
                    "related_entities": []
                }
            }
...
async def test_extrapolation_revision_loop(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.workflow.extrapolation_workflow.UniverseService", lambda: mock_uni_service
    )

    mock_retriever = MagicMock()
    mock_retriever.get_claims_dataset.return_value = "Some claims"
    monkeypatch.setattr(
        "app.workflow.extrapolation_workflow.KnowledgeRetrieverService",
    ...
    monkeypatch.setattr(
        "app.services.execution_service.ExecutionService.log_transition",
        lambda *_, **__: None,
    )
    monkeypatch.setattr(
        "app.services.theory_service.TheoryService.upsert_theory", lambda *_, **__: None
    )
...
async def test_architecture_node_dataset_aggregation(monkeypatch):
    ...
    monkeypatch.setattr(
        "app.workflow.tiering_workflow.UniverseService", lambda: mock_uni_service
    )

    mock_ret = MagicMock()
    dataset_values = ["Claims1", "Claims2"]
    def mock_get_dataset(_uid):
        return dataset_values.pop(0) if dataset_values else "Default"
    mock_ret.get_claims_dataset.side_effect = mock_get_dataset
    monkeypatch.setattr(
        "app.workflow.tiering_workflow.KnowledgeRetrieverService", lambda: mock_ret
    )

    monkeypatch.setattr(
        "app.workflow.tiering_workflow.run_agent",
    ...
    monkeypatch.setattr(
        "app.workflow.tiering_workflow._audit_tier_system",
    ...
    monkeypatch.setattr(
        "app.services.tiering_service.TieringService.create_rubric",
        lambda _self, defn: TierSystem(name="Rubric", id=100, system_definition=defn),
    )
    monkeypatch.setattr(
        "app.services.execution_service.ExecutionService.log_transition",
        lambda *_, **__: None,
    ...
    monkeypatch.setattr(
        "app.services.tiering_service.TieringService.slot_world",
        lambda *_, **__: None,
    )
