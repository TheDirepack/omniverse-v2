...
describe("DashboardPanel", () => {
	...
	it("calls startOrchestrate with comma-separated worlds on Run click", async () => {
		...
		const textarea = screen.getByPlaceholderText(
			"Warhammer 40k, Star Wars, Harry Potter",
		);
		await user.type(textarea, "Warhammer 40k, Star Wars");
		await user.click(screen.getByRole("button", { name: /run/i }));
		expect(api.startOrchestrate).toHaveBeenCalledWith([
			"Warhammer 40k",
			"Star Wars",
		]);
	});
	...
	it("calls researchUnexplored on Research button click", async () => {
		vi.mocked(api.researchUnexplored).mockResolvedValue({
			run_id: null,
			worlds: [],
			status: "ok",
		});
		...
		render(<DashboardPanel />);
		await user.click(
			screen.getByRole("button", { name: /research all unexplored/i }),
		);
	...
	it("calls runFocusedSearch with world name and feature", async () => {
		...
		expect(api.runFocusedSearch).toHaveBeenCalledWith(
			"Star Wars",
			"The Force is real",
		);
	});
	...
	it("displays SSE logs in Live Logs panel", async () => {
		...
		await waitFor(() =>
			expect(api.createEventSource).toHaveBeenCalledWith("run-1"),
		);
	...
	it("shows unexplored count in Research button label", async () => {
		...
		expect(
			screen.getByText(/research all unexplored.*2 worlds/i),
		).toBeInTheDocument();
	});
	...
	it("filters worlds by search term", async () => {
		...
		await user.type(searchInput, "Warhammer");
		expect(screen.getByText("Warhammer 40k")).toBeInTheDocument();
		expect(screen.queryByText("Star Wars")).not.toBeInTheDocument();
		expect(screen.queryByText("Warframe")).not.toBeInTheDocument();
	});
	...
	it("displays correct pipeline description help text", async () => {
		...
		const helpText = screen.getByText(
			/research \u2192 integration \u2192 summary pipeline/i,
		);
	...
	it("calls resetAllExplored when Reset All Explored Flags clicked", async () => {
		...
		render(<DashboardPanel />);
		await user.click(
			screen.getByRole("button", { name: /reset all explored flags/i }),
		);
