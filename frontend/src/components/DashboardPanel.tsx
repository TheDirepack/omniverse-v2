...
import type { LogEntry, WorldRecord } from "../types";
...
function DashboardPanel() {
	...
	const refreshWorlds = useCallback(async () => {
		try {
			setWorldRegistry(await api.fetchWorlds());
		} catch (e) {
			console.error(e);
		}
	}, []);
	...
	useEffect(() => {
		const init = async () => {
			try {
				const { active_runs, logs: recentLogs } =
					await api.fetchAgentActivity();
	...
	const unexploredCount = useMemo(
		() => worldRegistry.filter((w) => !w.is_explored).length,
		[worldRegistry],
	);
	const filteredWorlds = useMemo(() => {
		if (!worldSearch) return worldRegistry;
		return worldRegistry.filter((w) =>
			w.name.toLowerCase().includes(worldSearch.toLowerCase()),
		);
	}, [worldRegistry, worldSearch]);
	const displayWorlds = showAllWorlds
		? filteredWorlds
		: filteredWorlds.slice(0, 24);
	const hasMore = filteredWorlds.length > 24 && !showAllWorlds;

	const handleStartRun = async () => {
		const payload = worldsInput
			.split(",")
			.map((v) => v.trim())
			.filter(Boolean);
		if (!payload.length) return;
		try {
			setRunId((await api.startOrchestrate(payload)).run_id);
		} catch (e) {
			console.error(e);
		}
	};
	...
	const handleAddWorld = async () => {
		...
		setSaving(true);
		try {
			await api.addWorld(newWorldName.trim());
			setNewWorldName("");
			await refreshWorlds();
		} finally {
			setSaving(false);
	...
	const handleResetWorldExplored = async (worldId: number) => {
		await api.resetWorldExplored(worldId);
		await refreshWorlds();
	};
	...
	const handleDeleteWorld = async (worldId: number) => {
		if (
			!confirm(
				"Are you sure you want to delete this world? All associated data will be lost.",
			)
		)
			return;
		try {
	...
	const handleFocusedSearch = async () => {
		const worlds = focusedWorld
			.split(",")
			.map((v) => v.trim())
			.filter(Boolean);
		const features = focusedFeature
			.split(",")
			.map((v) => v.trim())
			.filter(Boolean);
		if (!worlds.length || !features.length) return;
		try {
			setRunId((await api.runFocusedSearch(worlds, features)).run_id);
		} catch (e) {
			console.error(e);
		}
	};
		...
		<>
			<section className="panel-grid">
				<div className="panel">
					...
					<p className="help-text">
						Comma-separated world names. Each one runs the full research →
						integration → summary pipeline.
					</p>
					<div className="button-row">
						{running ? (
							<button className="chip delete" onClick={handleStopRun}>
								Stop Research
							</button>
						) : (
							<>
				...
				<div className="panel terminal">
					...
					>
						...
						<div style={{ display: "flex", gap: 8, alignItems: "center" }}>
							<span
								className={`status-pill ${running ? "running" : logs.length > 0 ? "idle" : "idle"}`}
							>
								{running
									? "Running"
							...
							<button className="chip" onClick={handleClearLogs}>
								Clear Logs
							</button>
						</div>
					</div>
			...
			<section className="panel-grid" style={{ marginTop: 20 }}>
				<div className="panel">
					...
					<p className="help-text">
						Manage which worlds exist in the database. Click a world chip to
						toggle its explored flag.
					</p>
					<input
						value={worldSearch}
					...
					/>
					<button className="chip" onClick={handleResetAllExplored}>
						Reset All Explored Flags
					</button>
					<div className="chips">
						{displayWorlds.map((world) => (
							...
							>
								<button
									className={world.is_explored ? "chip active" : "chip"}
									onClick={() => void handleResetWorldExplored(world.id)}
								>
								...
								<button
									...
									onClick={() => handleSelectWorldForFocusedSearch(world.name)}
								>
									🔍
								</button>
								<button
						...
						{hasMore && (
							<button className="chip" onClick={() => setShowAllWorlds(true)}>
								+{filteredWorlds.length - 24} more
							</button>
						)}
					</div>
				</div>
				<div className="panel">
					...
					<p className="help-text">
						Prove or disprove specific features about worlds.
					</p>
					<textarea
						value={focusedWorld}
						onChange={(e) => setFocusedWorld(e.target.value)}
						placeholder="World names (comma-separated)"
					/>
					<textarea
						value={focusedFeature}
						onChange={(e) => setFocusedFeature(e.target.value)}
						placeholder="Features to prove/disprove (comma-separated)"
					/>
					<button
						className="primary"
					...
					<p className="muted">
						Reset research data and flags, keep settings and worlds.
					</p>
					<button className="primary" onClick={handleResetDB}>
						Reset DB
					</button>
				</div>
			</section>
