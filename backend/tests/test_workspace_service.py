...
from app.db.unconfirmed_schema import UnconfirmedClaim, NotebookEntry, ResearchSource
from app.core.domain import ResearchWorkspace

@pytest.mark.asyncio
async def test_workspace_notebook_isolation(clean_db):
...
@pytest.mark.asyncio
async def test_workspace_claim_isolation(clean_db):
    service = WorkspaceService()
    run_id_1 = "run-1"
    run_id_2 = "run-2"
...
@pytest.mark.asyncio
async def test_workspace_source_management(clean_db):
    service = WorkspaceService()
    universe_uuid = "uni-123"
    ...
    # Update source
    updated = service.upsert_source(universe_uuid, url, title="Title 2", source_id=source.id)
    assert updated.id == source.id
    assert updated.title == "Title 2"

@pytest.mark.asyncio
async def test_workspace_timeline_events(clean_db):
    service = WorkspaceService()
    universe_uuid = "uni-123"
    ...
    from app.db.unconfirmed_session import get_unconfirmed_session
    with get_unconfirmed_session() as session:
        from app.db.unconfirmed_schema import TimelineParticipant, TimelineLocation
        p = session.exec(select(TimelineParticipant).where(TimelineParticipant.timeline_id == event.id)).first()
        l = session.exec(select(TimelineLocation).where(TimelineLocation.timeline_id == event.id)).first()
        assert p is not None
        assert p.role == "Creator"
        assert l is not None

@pytest.mark.asyncio
async def test_workspace_indexing(clean_db):
    service = WorkspaceService()
    universe_uuid = "uni-idx"
    
    # Setup: Notebook, Source, Timeline
    service.upsert_notebook_entry(universe_uuid, "Note A", "Sum A", priority=10)
    service.upsert_source(universe_uuid, "http://src.com", title="Src A", reliability="High")
