...
- Synthesis now receives the canonical immutable target scope and provenance-safe evidence-scope metadata. Post-response scope and evidence validation remains strict.
- Extraction now rejects every invalid fragment independently while retaining valid fragments. It records each rejection in the durable checkpoint and emits an `evidence.fragment.rejected` warning event. Semantically malformed synthesis proposals now raise typed agent-output errors and conclude as durable target-level `PARTIAL` outcomes with inspectable gaps rather than terminal failures. In one routing invocation, a credential cooled after a retryable error is skipped when provider-target expansion tries later compatible models.
- Strict structured-output validation now makes one bounded repair request before falling through to the next eligible model or route candidate. Provider-target candidates retain sibling-model expansion after a failed repair. Repair context limits account for the complete serialized request and fail closed on overflow; repair start, success, and failure telemetry includes repair-tagged provider attempts.
- The internal planner bounds source budgets and aggregate planned scout fanout to eight candidate slots to preserve the ten-minute workflow lease. Users cannot configure this constraint.

## Wiki-first acquisition, knowledge, and operational hardening (2026-08-02)

Completed without a new live research run; all evidence is deterministic or cached-reference.

- **Wiki-first research foundation.** V2 now persists qualified wiki profiles by world/scope, caches same-host sitemap inventories, deterministically selects relevant pages by title/alias/section terms, and queues them. External discovery/search is deferred while relevant wiki pages remain, so generic search is target-level only. Alembic migration `v2_0007_wiki_research_foundation` adds profiles, inventory, queues, and reusable workspace knowledge.
- **Shared research context**: a page may answer many questions. Extracted, cited fragments source-backed workspace `WorkspaceKnowledgePublication` records published for subsequent planning/synthesis; they remain non-canon and require current-question targeting before reuse as answer evidence. Pages with no usable excerpt are not published.
- **MiniCPM preserves the whole document**: the preprocessor now reformats the entire cleaned document rather than only the selected passages, so a low-value/title-only view can never silently collapse the rest of the usable page. Per-passage guards remain; the previously title-only/extract drop path is gone.
- **Long caching**: content/inventory/search freshness defaulted to one week (`DEFAULT_CACHE_TTL_SECONDS`, `OMNIVERSE_V2_CACHE_TTL_SECONDS`, default 7*24*3600). `AcquisitionPolicy.freshness_seconds`, wiki inventory freshness, and `CachedFallbackSearch.ttl_seconds` all read this knob (minimum 24h, configurable to a week).
- **Canon enforced only at the final verification boundary**: the synthesis/audit steps may theorize provisionally (structural/safety invariants remain), and the integrate/completion gate enforces promotion/independence; proposals rejected at verification are pushed back as actionable gaps (`REJECTED_AT_VERIFY`) that keep the run PARTIAL and re-targetable, not silent COMPLETE.
- **Planner may issue generic web search** while the service still governs acquisition and keeps a wiki-first strategy; the planner prompt no longer forbids generic web research.
- **Models tab reordering**: user-controllable `provider_model.sort_order` with HTMX UP/DOWN move controls; reordering the models list does not change strict route/fallback positions. Alembic migration `v2_0008_provider_model_sort_order`.
- **Search separated from LLM providers**: `BRAVE_SEARCH` and non-LLM provider kinds no longer appear as LLM providers in Settings/models; search credentials remain configurable through the dedicated flow.
- **Deterministic + live/cached-reference tests**: `backend/tests_v2/cached_references` + `fixtures/` seed real fetched provider/wiki responses as stable offline references; `reference_cache`/`cached_reference` fixtures read only the cache. `live`/`slow`/`evaluation` markers are deselected by default, and explicit live tests may fetch the network.
