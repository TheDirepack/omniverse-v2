...
async def test_trigger_tiering_endpoint():
    """Test that /api/tiering returns a run_id and queues the task."""
    with patch("app.api.routes.run_tiering_in_background", new=AsyncMock()) as mock_task:
        response = client.post("/tiering")
