...
export async function fetchSettings(): Promise<{
	general_settings: Record<string, string | null>;
...
export async function fetchUnconfirmedTraits(
	universeNames?: string[],
): Promise<UnconfirmedTrait[]> {
	const query = universeNames ? `?universe_ids=${universeNames.join(",")}` : "";
	return apiFetch(`/api/research/traits/unconfirmed${query}`);
}

export async function fetchUnconfirmedClaims(
	universeNames?: string[],
): Promise<UnconfirmedClaim[]> {
	const query = universeNames ? `?universe_ids=${universeNames.join(",")}` : "";
...
export async function researchUnexplored(): Promise<{
	run_id: string | null;
	worlds: string[];
	status: string;
}> {
...
export async function runFocusedSearch(
	worlds: string[],
	features: string[],
): Promise<{ run_id: string }> {
...
export async function deleteAgentRoute(routeId: number): Promise<any> {
	return apiFetch(`/api/settings/agent-routes/${routeId}`, {
		method: "DELETE",
	});
}
...
export async function fetchAgentActivity(): Promise<{
	active_runs: string[];
	logs: LogEntry[];
}> {
...
export async function extrapolate(payload: {
	scope: "all" | "worlds" | "tier";
	worlds?: string[];
	tier?: number;
}): Promise<{ run_id: string; worlds: string[] }> {
...
export async function triggerRuleProposal(): Promise<{
	run_id: string;
	status: string;
}> {
...
export async function triggerMaterialization(): Promise<{
	status: string;
...
export async function fetchCompositionDepth(): Promise<{
	max_composition_depth: number;
}> {
	return apiFetch("/api/inference/depth");
}

export async function setCompositionDepth(
	depth: number,
): Promise<{ max_composition_depth: number }> {
	return apiFetch("/api/inference/depth", {
		method: "PUT",
