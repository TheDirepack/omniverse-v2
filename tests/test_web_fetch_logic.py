...
from app.core.web_fetch import WebFetcher, COOKIE_PATTERNS, NOISE_TAGS, NOISE_SELECTORS
...
def test_extract_internal_links(fetcher):
    ...
    # Test default (max 20)
    links = fetcher._extract_internal_links(soup)
    
    # Fusion Engine should be High value and have score 2, and section "Technical Specifications"
    assert "Fusion Engine" in links
    assert "[High Value | 2x | Technical Specifications]" in links
    assert "/fusion-engine" in links
    
    # Heat Sink should be High value and score 1
    assert "Heat Sinks" in links
    assert "[High Value | 1x | Technical Specifications]" in links
    
    # Categories should be Medium value
    assert "Mechs Category" in links
    assert "[Medium Value" in links
    
    # Junk links should be filtered
    assert "Privacy Policy" not in links
    assert "About Us" not in links
...
def test_extract_internal_links_max(fetcher):
    ...
    # Test capping at 2
    links = fetcher._extract_internal_links(soup, max_links=2)
    # Count the number of lines starting with '-'
    link_count = len([line for line in links.splitlines() if line.startswith("-")])
    assert link_count == 2
