...
async def test_research_single_world_integration_flow(mocker, clean_db):
    ...
    mock_run.side_effect = [
        (
            True,
            '{"Universe_Name": "IntegrationWorld", "Verified_Claims": [], '
            '"Knowledge_Graph": [], "Missing_Info": [], "Provisional_Conclusions": []}',
            [],
        ),
        (True, "STATUS: SUCCESS", []),
        (True, "STATUS: SUCCESS", []),
    ...
    mock_retriever = mocker.patch(
        "app.services.knowledge_retriever.KnowledgeRetrieverService",
    ...
    mock_uni_service = mocker.patch(
        "app.services.universe_service.UniverseService", return_value=mocker.Mock()
    )
    mock_uni_service.return_value.get_universe_by_uuid.return_value = Universe(
        name="IntegrationWorld", id=1
    )
...
async def test_research_single_world_min_turns_passed(mocker, clean_db):
    ...
    mock_run.return_value = (
        True,
        '{"Universe_Name": "MinTurnsWorld", "Verified_Claims": [], '
        '"Knowledge_Graph": [], "Missing_Info": [], "Provisional_Conclusions": []}',
        [],
    )
    ...
    _args, kwargs = mock_run.call_args_list[0]
    assert "max_turns" in kwargs
    # The prompt uses min_turns internally in the loop logic but passed to
    # run_agent as max_turns
