...
async def research_node(state: OmniverseState) -> dict[str, Any]:
    ...
    if not verified_worlds:
        ...
        return {
            ...
            "errors": errors,
            "active_task": "FACILITATION",
        }
 
    next_task = "FACILITATION"
    return {
        "research_results": successful_results,
        "verified_worlds": verified_worlds,
        "errors": errors,
...
async def facilitation_node(state: OmniverseState) -> dict[str, Any]:
    run_id = state.get("run_id")
    ...
    exec_service.log_transition(
        run_id,
        "Facilitator",
        f"Sifting results for {len(research_results)} worlds",
        "FACILITATING",
        state,
    )
    
    from app.agents.prompts import get_facilitator_prompt
    ...
    for result in research_results:
        ...
        if success:
            try:
                # Update result with graduated claims
                facilitated_data = json.loads(final_ans)
                result["graduated_claims"] = facilitated_data.get("graduated_claims", [])
                result["retained_claims"] = facilitated_data.get("retained_claims", [])
                result["facilitation_summary"] = facilitated_data.get("decision_summary", "")
            except json.JSONDecodeError:
                result["facilitation_error"] = "Failed to parse facilitator output"
        else:
            result["facilitation_error"] = f"Facilitation failed: {final_ans}"
        
        facilitated_results.append(result)
    
    exec_service.log_transition(
        run_id, "Facilitator", "Sifting complete. Passing to DB Integrator.", "COMPLETED", state
    )
    
    return {
        "research_results": facilitated_results,
        "active_task": "DB_INTEGRATION"
    }
