...
from app.db.schema import Universe, Entity, Claim, Evidence, EvidenceChunk, Predicate
from app.services.knowledge_retriever import KnowledgeRetrieverService
from app.core.tools import tool_query_claims, tool_upsert_claims, tool_web_search, tool_fetch_page
...
async def test_tool_upsert_claims_evidence(setup_universe):
    ...
    with Session(engine) as session:
        ...
        evidence = session.exec(select(Evidence).where(Evidence.source_url == "http://wiki.test/Entity1")).first()
        assert evidence is not None
        
        chunk = session.exec(select(EvidenceChunk).where(EvidenceChunk.evidence_id == evidence.id)).first()
        assert chunk is not None
        assert chunk.content == "Section 2"
        
        # Check if claim is linked to chunk
        claim = session.exec(select(Claim).where(Claim.predicate == "HAS_POWER")).first()
        assert claim is not None
        assert claim.evidence_chunk_id == chunk.id

@pytest.mark.asyncio
async def test_tool_web_search_parallel_multi_engine():
    with patch("app.core.tools.web_searcher.perform_search", new_callable=AsyncMock) as mock_search:
...
async def test_tool_fetch_page_parallel():
    with patch("app.core.tools.web_fetcher.fetch_page", new_callable=AsyncMock) as mock_fetch:
