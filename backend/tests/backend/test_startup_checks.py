...
def test_reconcile_stale_runs(_ephemeral_db):
    ...
    with Session(engine) as session:
        # Check Run 1
        latest_1 = session.exec(
            select(ExecutionState)
            .where(ExecutionState.run_id == "run-completed")
            .order_by(ExecutionState.created_at.desc())
        ).first()
        assert latest_1.status == "COMPLETED"

        # Check Run 2
        latest_2 = session.exec(
            select(ExecutionState)
            .where(ExecutionState.run_id == "run-stale")
            .order_by(ExecutionState.created_at.desc())
        ).first()
        ...
        # Check Run 3
        latest_3 = session.exec(
            select(ExecutionState)
            .where(ExecutionState.run_id == "run-stale-2")
            .order_by(ExecutionState.created_at.desc())
        ).first()
        assert latest_3.status == "FAILED"

        # Check Run 4
        latest_4 = session.exec(
            select(ExecutionState)
            .where(ExecutionState.run_id == "run-failed")
            .order_by(ExecutionState.created_at.desc())
        ).first()
        assert latest_4.status == "FAILED"

def test_validate_settings_errors(_ephemeral_db):
...
def test_validate_settings_valid(_ephemeral_db):
