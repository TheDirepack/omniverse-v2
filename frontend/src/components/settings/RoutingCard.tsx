...
function RoutingCard({
	...
	const statusText = isDefault
		? `${routes.length} step(s)`
		: hasCustom
			? `${routes.length} custom step(s)`
			: `Using DEFAULT (${defaultCount} step(s))`;
	...
	const addModelTag = (idx: number, model: string) => {
		...
		const currentModels = (localRoutes[idx].models || "")
			.split(",")
			.map((m) => m.trim())
			.filter(Boolean);
	...
	const resolvedSequence: Array<{
		provider: string;
		keyLabel: string;
		model: string;
	}> = [];
	const routesToResolve = (
		localRoutes.length > 0 ? localRoutes : (defaultRoutes ?? [])
	)
		.slice()
	...
	routesToResolve.forEach((route) => {
		...
		if (!provider) return;
		const models = (route.models || provider.models || "")
			.split(",")
			.map((m) => m.trim())
			.filter(Boolean);

		const keys =
			provider.keys && provider.keys.length > 0 ? provider.keys : [{}];
			...
			{expanded && (
				<div className="routing-card-body">
					<p className="help-text">
						Each agent tries its fallback chain top to bottom: provider → its
						keys in order → its models in order. If everything fails, the run
						fails that step. Agents without their own chain use the DEFAULT
						chain.
					</p>
					...
					<div className="route-slots">
						{localRoutes.map((route, idx) => {
							const providerModels =
								providers.find((p) => p.id === route.provider_id)?.models ?? "";
							return (
								<div
									key={route.id}
											...
											{(() => {
												...
												const modelList = providerModels
													? providerModels
															...
															.filter(Boolean)
													: [];
												const selectedModels = (route.models || "")
													.split(",")
													.map((m) => m.trim())
													.filter(Boolean);
												return (
													<div className="models-tags">
														...
														<div className="tag-input-wrap">
															...
															<button
																className="chip"
																onClick={() =>
																	addModelTag(idx, modelDrafts[idx] || "")
																}
								...
								<ol className="sequence-list">
									{resolvedSequence.map((item, idx) => (
										<li key={idx} className="sequence-item">
											{item.provider} → {item.keyLabel} → {item.model}
										</li>
									))}
								</ol>
							)}
